...
import com.vdc.tv.player.core.domain.models.PlayerChapter
...
import com.vdc.tv.player.local.domain.PlaylistManager
...
constructor(
    ...
    fun addSubtitle(url: String, title: String?, language: String?) {
        val currentTitle = title ?: application.getString(R.string.external)
        val currentLanguage = language ?: ""

        // 1. Add to low-level player
        (player as? MPVPlayer)?.subAdd(url, currentTitle, currentLanguage)

        // 2. Update internal state so UI refreshes
        val currentId = player.currentMediaItem?.mediaId ?: return
        val itemIndex = items.indexOfFirst { it.itemId.toString() == currentId }
        if (itemIndex != -1) {
            val currentItem = items[itemIndex]
            val newSubtitle = ExternalSubtitle(
                title = currentTitle,
                language = currentLanguage,
                uri = Uri.parse(url),
                mimeType = "text/srt" // Most remote subtitles are SRT
            )

            val updatedItem = currentItem.copy(
                externalSubtitles = currentItem.externalSubtitles + newSubtitle
            )
            items[itemIndex] = updatedItem

            // 3. Update Media3 player to refresh track selector
            val currentPosition = player.currentPosition
            val wasPlaying = player.isPlaying
            val isMpv = player is MPVPlayer

            player.replaceMediaItem(player.currentMediaItemIndex, updatedItem.toMediaItem())

            // For ExoPlayer, we need to call prepare() to force it to re-read the subtitle configurations
            if (!isMpv) {
