...
class MPVPlayer(
    ...
    fun subAdd(url: String, title: String? = null, language: String? = null) {
        Timber.d("Adding subtitle: url=$url, title=$title, lang=$language")
        MPVLib.command(arrayOf("sub-add", url, "select", title ?: "Extern", language ?: ""))
        // Force the title property on the last added track if possible
        handler.postDelayed({
            try {
                val trackListStr = MPVLib.getPropertyString("track-list") ?: return@postDelayed
                val trackList = JSONArray(trackListStr)
                var lastSubId = -1
                for (i in 0 until trackList.length()) {
                    val track = trackList.getJSONObject(i)
                    if (track.optString("type") == "sub") {
                        lastSubId = track.optInt("id")
                    }
                }
                if (lastSubId != -1) {
                    MPVLib.command(arrayOf("track-set", "$lastSubId", "title", title ?: "Extern"))
                }
            } catch (e: Exception) {
                Timber.e(e, "Failed to set subtitle title")
            }
        }, 500)
    }
    ...
    override fun replaceMediaItems(
        ...
        internalMediaItems.removeAll { internalMediaItems.indexOf(it) in range }
        internalMediaItems.addAll(fromIndex, mediaItems)
        listeners.sendEvent(EVENT_TIMELINE_CHANGED) { listener ->
            listener.onTimelineChanged(
                timeline,
                TIMELINE_CHANGE_REASON_PLAYLIST_CHANGED,
    ...
    companion object {
        ...
        private fun getTracks(trackList: String): Tracks {
            Timber.d("MPV track-list changed: $trackList")
