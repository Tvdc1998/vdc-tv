...
import com.vdc.tv.film.presentation.library.LibraryState
import com.vdc.tv.film.presentation.library.LibraryViewType
import com.vdc.tv.film.presentation.library.LibraryViewModel
import com.vdc.tv.models.CollectionType
import com.vdc.tv.models.FindroidItem
import com.vdc.tv.presentation.components.ErrorDialog
import com.vdc.tv.presentation.film.components.Direction
import com.vdc.tv.presentation.film.components.ErrorCard
import com.vdc.tv.presentation.film.components.GenreDialog
import com.vdc.tv.presentation.film.components.ItemCard
import com.vdc.tv.presentation.film.components.SortByDialog
...
private fun LibraryScreenLayout(
    ...
    Scaffold(
        ...
        topBar = {
            TopAppBar(
                ...
                actions = {
                    IconButton(onClick = { onAction(LibraryAction.ToggleFilter(ItemFilter.IS_UNPLAYED)) }) {
                        Icon(
                            painter = painterResource(if (state.filters.contains(ItemFilter.IS_UNPLAYED)) CoreR.drawable.ic_eye_off else CoreR.drawable.ic_eye),
                            contentDescription = null,
                            tint = if (state.filters.contains(ItemFilter.IS_UNPLAYED)) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurface
                        )
                    }
                    if (state.genres.isNotEmpty()) {
                        IconButton(onClick = { showGenreDialog = true }) {
                            Icon(
                                ...
                                contentDescription = null,
                                tint = if (state.genreIds.isNotEmpty()) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurface
                            )
                        }
                    }
                    IconButton(onClick = { onAction(LibraryAction.ToggleViewType) }) {
                        Icon(
                            painter = painterResource(if (state.viewType == LibraryViewType.POSTER) CoreR.drawable.ic_layout_dashboard else CoreR.drawable.ic_film),
                            contentDescription = null,
                        )
                    }
        ...
        Column {
            ...
            LazyVerticalGrid(
                columns = if (state.viewType == LibraryViewType.POSTER) {
                    GridCellsAdaptiveWithMinColumns(minSize = 160.dp, minColumns = 2)
                } else {
                    GridCellsAdaptiveWithMinColumns(minSize = 260.dp, minColumns = 1)
                },
    ...
    if (showGenreDialog) {
        GenreDialog(
            ...
            onToggleGenre = { onAction(LibraryAction.ToggleGenre(it)) },
