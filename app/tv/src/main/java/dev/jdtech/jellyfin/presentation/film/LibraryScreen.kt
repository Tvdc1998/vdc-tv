...
private fun LibraryScreenLayout(
    ...
    LazyVerticalGrid(
        columns = if (state.viewType == LibraryViewType.POSTER) GridCells.Fixed(5) else GridCells.Fixed(3),
        ...
        item(span = { GridItemSpan(this.maxLineSpan) }) {
            Row(
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(MaterialTheme.spacings.small)) {
                    Button(onClick = { onAction(LibraryAction.ToggleViewType) }) {
                        Icon(
                        ...
                        Text(text = stringResource(if (state.viewType == LibraryViewType.POSTER) CoreR.string.thumbnails else CoreR.string.posters))
                    }
