...
constructor(
    ...
    private fun loadGenres() {
        viewModelScope.launch {
            try {
                ...
                _state.emit(_state.value.copy(genres = genres))
            } catch (_: Exception) {}
        }
    }
                ...
                val items =
                    jellyfinRepository
                        .getItemsPaging(
                            ...
                            genreIds = _state.value.genreIds,
                        )
    ...
    private suspend fun initSorting() {
        if (!::sortBy.isInitialized || !::sortOrder.isInitialized) {
            val sortByString = try { appPreferences.getValue(appPreferences.sortBy) } catch (e: Exception) { "SortName" }
            val sortOrderString = try { appPreferences.getValue(appPreferences.sortOrder) } catch (e: Exception) { "Ascending" }
            
            sortBy = SortBy.fromString(sortByString)
            sortOrder = SortOrder.fromString(sortOrderString)
    ...
    fun onAction(action: LibraryAction) {
        when (action) {
            ...
            is LibraryAction.ToggleFilter -> {
                val currentFilters = _state.value.filters.toMutableList()
                if (currentFilters.contains(action.filter)) {
                    currentFilters.remove(action.filter)
                } else {
                    currentFilters.add(action.filter)
                }
                viewModelScope.launch {
                    _state.emit(_state.value.copy(filters = currentFilters))
                    loadItems()
                }
            }

            is LibraryAction.ToggleGenre -> {
                val currentGenreIds = _state.value.genreIds.toMutableList()
                if (currentGenreIds.contains(action.genreId)) {
                    currentGenreIds.remove(action.genreId)
                } else {
                    currentGenreIds.add(action.genreId)
                }
                viewModelScope.launch {
                    _state.emit(_state.value.copy(genreIds = currentGenreIds))
                    loadItems()
                }
            }

            is LibraryAction.ToggleViewType -> {
                val newViewType = if (_state.value.viewType == LibraryViewType.POSTER) {
                    LibraryViewType.THUMBNAIL
                } else {
                    LibraryViewType.POSTER
                }
                viewModelScope.launch {
                    _state.emit(_state.value.copy(viewType = newViewType))
                }
            }
